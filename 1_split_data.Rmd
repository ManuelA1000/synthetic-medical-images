...
images_all <- split_1 %>%
    ...
    drop_na() %>%
    filter(image_id != '49318.bmp') ## eye speculum image

images_no <- images_all %>%
    filter(ground_truth == 'No' & view == 'posterior') # %>%
    # sample_n(1226)

images_preplus <- images_all %>%
    filter(ground_truth == 'Pre-Plus' & view == 'posterior')

images_plus <- images_all %>%
    filter(ground_truth == 'Plus' & view == 'posterior')
```


```{r}
gan_no2plus <- images_no %>%
    bind_rows(images_plus)

gan_no2preplus <- images_no %>%
    bind_rows(images_preplus)

gan_preplus2plus <- images_preplus %>%
    bind_rows(images_plus)
```
...
```{r}
cnn_train <- split_3 %>%
    bind_rows(split_1, split_2) %>% ## comment out to only train on split 3
    drop_na(posterior)
...
file_copy(paste(src, gan_no2plus$image_id, sep = '/'),
          paste(dst, gan_no2plus$ground_truth, gan_no2plus$image_id, sep = '/'),
          overwrite = TRUE)

dst = './out/gan/no2preplus/images'
file_copy(paste(src, gan_no2preplus$image_id, sep = '/'),
          paste(dst, gan_no2preplus$ground_truth, gan_no2preplus$image_id, sep = '/'),
          overwrite = TRUE)

dst = './out/gan/preplus2plus/images'
file_copy(paste(src, gan_preplus2plus$image_id, sep = '/'),
          paste(dst, gan_preplus2plus$ground_truth, gan_preplus2plus$image_id, sep = '/'),
          overwrite = TRUE)

dst = './out/cnn/train/real'
file_copy(paste(src, cnn_train$posterior, sep = '/'),
          paste(dst, cnn_train$ground_truth, cnn_train$posterior, sep = '/'),
          overwrite = TRUE)

dst = './out/cnn/val/real'
file_copy(paste(src, cnn_val$posterior, sep = '/'),
          paste(dst, cnn_val$ground_truth, cnn_val$posterior, sep = '/'),
          overwrite = TRUE)

dst = './out/cnn/test/real'
file_copy(paste(src, cnn_test$posterior, sep = '/'),
          paste(dst, cnn_test$ground_truth, cnn_test$posterior, sep = '/'),
          overwrite = TRUE)
```
